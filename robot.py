...
from controller import Robot, Motor, LED, DistanceSensor, Camera, CameraRecognitionObject
import sys
...
class ARAP:
    ...
    def reset_actuator_values(self):
        ...
        for i in range(self.GROUND_SENSORS_NUMBER):
            self.ground_sensors_values[i] = 0.0
    ...
    def get_ground_sensor_values(self):
        for i in range(self.GROUND_SENSORS_NUMBER):
            self.ground_sensors_values[i] = self.ground_sensors[i].getValue()
            g1 = self.ground_sensors_values[0]
            g2 = self.ground_sensors_values[1]
            g3 = self.ground_sensors_values[2]
